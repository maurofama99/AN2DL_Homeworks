...
class1 = Dense(1024, activation='relu', name='Hidden1')(flat1)
class2 = Dense(1024, activation='relu', name='Hidden2')(class1)
output = Dense(8, activation='softmax', name='Output')(class2)
# define new model
...
def count_imgs_per_class(dataset):
  imgsperclass = np.array([[0,0,0,0,0,0,0,0]])
  for element in dataset.as_numpy_iterator():
    a = np.array([element[1]])
    tmpimgsperclass=np.sum(a,axis=1)  
    imgsperclass = np.concatenate((imgsperclass,tmpimgsperclass),axis=0)
    imgsperclass = np.sum(imgsperclass, axis=0)
    imgsperclass = np.array([imgsperclass])
  
  return imgsperclass

classtrainds = count_imgs_per_class(train_ds)
print(classtrainds)

classes = np.arange(start=1, stop=9)
# Figure Size
fig = plt.figure(figsize =(15, 8))
 
# Horizontal Bar Plot
plt.bar(classes,classtrainds[0])
 
# Show Plot
plt.show()

valclasses = count_imgs_per_class(val_ds)
print(valclasses)

fig = plt.figure(figsize =(10, 8))
 
# Horizontal Bar Plot
plt.bar(classes,valclasses[0])
